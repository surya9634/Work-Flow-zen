...
const CreateCampaignModal = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  const validateStep = (stepIndex) => {
    ...
    switch (step.id) {
      ...
      case "persona":
        return (
          stepData.name.trim() && stepData.position.trim() && stepData.tone
        );
      case "leads":
    ...
    if (isValid) {
      ...
      try {
        ...
        if (campaignData.name && data?.campaign?.name !== campaignData.name) {
          try {
            await fetch(`/api/campaigns/${campId}`, {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
        ...
        const startRes = await fetch(`/api/campaigns/${campId}/start`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            conversationId: campaignData?.conversationId || null,
          }),
        });
        const startData = await startRes.json();
        if (!startRes.ok || !startData?.success)
          throw new Error(startData?.message || "start_failed");
        onSave({
          ...data.campaign,
          name: campaignData.name || data.campaign.name,
        });
        onClose();
      } catch (e) {
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <CurrentStepComponent
                  ...
                  onChange={(data) =>
                    updateCampaignData(STEPS[currentStep].id, data)
                  }
            ...
            <div className="flex space-x-3">
              {!isLastStep ? (
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-lg transition-all duration-200 text-white ${
                    isSubmitting
