...
function WhatsappChat() {
  const [chats, setChats] = useState([]); // [{id,name,lastMessage,time,status,lastMessageTime, campaignType, campaign}]
  ...
  // Filter States (Extended)
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('All');
  const [campaignTypeFilter, setCampaignTypeFilter] = useState('All Types'); // NEW
  const [campaignFilter, setCampaignFilter] = useState('All Campaigns');   // NEW
    ...
    (async () => {
      try {
        ...
        const normalized = waConvs.map(c => ({
          ...
          time: '',
          status: 'Active',
          campaignType: c.campaignType || 'Support', // Mocked type
          campaign: c.campaign || 'Product Launch', // Mocked campaign
  ...
  return (
    // The fixed height of the chat container h-[calc(100vh-4rem)] already accounts for a 4rem navbar.
    // The issue of the screen "jumping" or "scrolling up" is often related to the ChatWindow or ChatList
    // trying to force an element into view (e.g., auto-scrolling to the active chat in ChatList or
    // auto-scrolling to the bottom message in ChatWindow) without proper containment, causing the main
    // page body to shift. 
    // Since the parent container is fixed height, the problem is likely internal to the chat list or window.
    // We maintain the current height structure, which is the correct approach for a full-screen app area.
      ...
      <div className="w-80 border-r border-gray-200 flex flex-col h-full">
        ...
        <div className="flex-shrink-0">
          <ChatFilter
            ...
            campaignTypeFilter={campaignTypeFilter} // NEW
            setCampaignTypeFilter={setCampaignTypeFilter} // NEW
            campaignFilter={campaignFilter} // NEW
            setCampaignFilter={setCampaignFilter} // NEW
          />
        </div>
        {/* Chat List */}
        <div className="flex-1 overflow-hidden">
          <ChatList
            ...
            searchTerm={searchTerm}
            statusFilter={statusFilter}
            // Pass the new filters to ChatList for filtering logic
