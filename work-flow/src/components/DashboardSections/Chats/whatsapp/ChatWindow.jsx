...
const ChatWindow = ({ activeChat, messages, onSendMessage, aiMode, onToggleAI, onStatusChange, isDetailsOpen, onToggleDetails, theme = 'whatsapp' }) => {
  ...
  const scrollToBottom = () => {
    // FIX: Wrap in setTimeout to ensure DOM layout is complete before scrolling
    setTimeout(() => {
  ...
  useEffect(() => {
    // Only scroll to bottom when message count increases, not just on component mount/render
    scrollToBottom();
  }, [messages.length]); 
  ...
  return (
    // FIX: The outer container relies on the parent element (e.g., WhatsappChat) to define its fixed height.
    <div className="flex-1 flex flex-col bg-white h-full"> 
      ...
      {/* Messages Container with Auto-scroll (Correctly uses flex-1 for scroll containment) */}
      <div 
