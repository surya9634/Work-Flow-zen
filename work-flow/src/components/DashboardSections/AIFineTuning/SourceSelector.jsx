...
const SourceSelector = ({ onSourceSelect, showAsButton = false }) => {
  ...
  if (showAsButton) {
    // Note: Applying the gradient style to the dropdown menu items' background, 
    // but keeping the button itself the default blue for visibility and primary action feel.
    return (
      <div className="relative" ref={dropdownRef}>
        ...
        {showDropdown && (
          <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-gray-200 z-50 overflow-hidden" style={BLUE_GRADIENT_STYLE}>
            <div className="p-2">
              {sources.map((source) => (
                <button
                  key={source.type}
                  onClick={() => handleSourceClick(source.type)}
                  // The dropdown item backgrounds remain simple hover effects for best UX
  ...
  // Default grid mode - shows as the main selector grid (Gradient applied here)
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sources.map((source) => (
          <button
            ...
            className="border-2 border-gray-200 rounded-xl p-6 hover:border-blue-400 hover:shadow-lg transition-all duration-200 group text-left"
